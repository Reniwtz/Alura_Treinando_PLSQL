--1) O objetivo agora é atualizar todos os segmentos de mercados de todos os clientes atualmente cadastrados na tabela. Para isso, você tem que repetir a execução da procedure diversas vezes para cada um dos clientes:
...
--4) No código acima, é preciso conhecer, de antemão, o número de clientes existentes na tabela, neste momento. Mas isso pode ser feito pelo próprio programa PL/SQL:
DECLARE
   v_SEGMERCADO CLIENTE.SEGMERCADO_ID%type := 3;
   v_ID CLIENTE.ID%type := 1;
   v_NUMCLI INTEGER;
...
--5) Não temos à disposição apenas o comando de repetição LOOP ... END LOOP. Essa repetição pode ser feita com a estrutura de repetição FOR. Inclusive, é a mais apropriada quando temos o número de interações previamente definido:
DECLARE
